...
EditorSettingsDialog::EditorSettingsDialog(const EditorSettings &current,
                                           QWidget *parent)
    : QDialog(parent) {
...
void EditorSettingsDialog::setupRenderingTab(QWidget *tab,
                                             const EditorSettings &current) {
...
void EditorSettingsDialog::setupAITab(QWidget *tab,
                                      const EditorSettings &current) {
  auto *layout = new QVBoxLayout(tab);
  ...
  m_llmProvider->addItem(tr("Disabled (Procedural Only)"),
                         static_cast<int>(LLMProviderType::None));
  m_llmProvider->addItem(tr("OpenAI (GPT-4, DALL-E 3)"),
                         static_cast<int>(LLMProviderType::OpenAI));
  m_llmProvider->addItem(tr("Anthropic (Claude)"),
                         static_cast<int>(LLMProviderType::Anthropic));
  m_llmProvider->addItem(tr("Stability AI (Stable Diffusion)"),
                         static_cast<int>(LLMProviderType::StabilityAI));
  m_llmProvider->addItem(tr("Local Ollama"),
                         static_cast<int>(LLMProviderType::LocalOllama));
  m_llmProvider->addItem(tr("Custom Endpoint"),
                         static_cast<int>(LLMProviderType::Custom));

  int providerIndex =
      m_llmProvider->findData(static_cast<int>(current.llm.provider));
  if (providerIndex >= 0) {
  ...
  auto *testLLMBtn = new QPushButton(tr("Test LLM"), tab);
  connect(testLLMBtn, &QPushButton::clicked, this,
          &EditorSettingsDialog::onTestLLM);
  testRow->addWidget(testLLMBtn);

  auto *testImageBtn = new QPushButton(tr("Test Image Gen"), tab);
  connect(testImageBtn, &QPushButton::clicked, this,
          &EditorSettingsDialog::onTestImageGen);
  testRow->addWidget(testImageBtn);
...
void EditorSettingsDialog::onProviderChanged(int index) {
  auto provider =
      static_cast<LLMProviderType>(m_llmProvider->itemData(index).toInt());
  updateAIFieldsFromProvider(provider);
}

void EditorSettingsDialog::updateAIFieldsFromProvider(
    LLMProviderType provider) {
  bool enabled = provider != LLMProviderType::None;
  bool needsApiKey = provider != LLMProviderType::None &&
                     provider != LLMProviderType::LocalOllama;
  bool needsEndpoint = provider == LLMProviderType::Custom ||
                       provider == LLMProviderType::LocalOllama;
...
Assets::LLMConfig EditorSettingsDialog::buildLLMConfig() const {
  Assets::LLMConfig config;

  auto provider =
      static_cast<LLMProviderType>(m_llmProvider->currentData().toInt());
  ...
  case LLMProviderType::OpenAI:
    ...
    break;
  case LLMProviderType::Anthropic:
    config.provider = Assets::LLMProvider::Anthropic;
    break;
  case LLMProviderType::StabilityAI:
    config.provider = Assets::LLMProvider::StabilityAI;
    break;
  case LLMProviderType::LocalOllama:
    config.provider = Assets::LLMProvider::LocalOllama;
    break;
  case LLMProviderType::Custom:
    config.provider = Assets::LLMProvider::Custom;
    break;
  default:
    config.provider = Assets::LLMProvider::OpenAI;
    break;
  ...
  config.apiKey = m_apiKey->text().toStdString();
  config.endpoint = m_endpoint->text().toStdString();
  config.model = m_model->text().toStdString();
  config.imageModel = m_imageModel->text().toStdString();
  ...
  if (config.imageModel.empty()) {
    config.imageModel =
        Assets::LLMConfig::GetDefaultImageModel(config.provider);
  }

  return config;
}

void EditorSettingsDialog::onTestLLM() {
  auto provider =
      static_cast<LLMProviderType>(m_llmProvider->currentData().toInt());
  ...
  if (m_apiKey->text().isEmpty() && provider != LLMProviderType::LocalOllama) {
    QMessageBox::warning(this, tr("Test LLM"), tr("Please enter an API key."));
    return;
  }

  // Build config and create client
  auto config = buildLLMConfig();
  auto client = Assets::LLMClientFactory::Create(config);

  if (!client || !client->IsReady()) {
    QMessageBox::warning(this, tr("Test LLM"),
                         tr("Failed to initialize LLM client."));
    return;
  }
  ...
  Assets::LLMTextRequest request;
  request.userPrompt = "Say hello in exactly one word.";
  request.maxTokens = 64;
  request.temperature = 0.7f;

  auto response = client->GenerateText(request);

  setCursor(Qt::ArrowCursor);

  if (response.success) {
    QString msg =
        tr("✓ LLM Connection Successful!\n\n") +
        tr("Model: %1\n").arg(QString::fromStdString(config.model)) +
        tr("Response: %1\n").arg(QString::fromStdString(response.content)) +
        tr("Latency: %1 ms").arg(response.latencyMs);
    QMessageBox::information(this, tr("Test LLM"), msg);
  } else {
    QString msg =
        tr("✗ LLM Connection Failed\n\n") +
        tr("Error: %1").arg(QString::fromStdString(response.errorMessage));
    QMessageBox::warning(this, tr("Test LLM"), msg);
  }
}

void EditorSettingsDialog::onTestImageGen() {
  auto provider =
      static_cast<LLMProviderType>(m_llmProvider->currentData().toInt());

  if (provider == LLMProviderType::None) {
    QMessageBox::information(
        this, tr("Test Image Gen"),
        tr("AI generation is disabled. Select a provider first."));
    return;
  }

  // Only OpenAI and Stability AI support image generation
  if (provider != LLMProviderType::OpenAI &&
      provider != LLMProviderType::StabilityAI) {
    QMessageBox::information(this, tr("Test Image Gen"),
                             tr("Image generation is only supported by OpenAI "
                                "and Stability AI providers."));
    return;
  }

  if (m_apiKey->text().isEmpty()) {
    QMessageBox::warning(this, tr("Test Image Gen"),
  ...
  if (!client || !client->IsReady()) {
    QMessageBox::warning(this, tr("Test Image Gen"),
                         tr("Failed to initialize image generation client."));
    return;
  }
  ...
  Assets::LLMImageRequest request;
  request.prompt = "A simple red cube on white background";
  request.width = 256;
  request.height = 256;
  request.numImages = 1;

  auto response = client->GenerateImage(request);

  setCursor(Qt::ArrowCursor);

  if (response.success) {
    QString msg =
        tr("✓ Image Generation Successful!\n\n") +
        tr("Model: %1\n").arg(QString::fromStdString(config.imageModel)) +
        tr("Images generated: %1\n").arg(response.images.size()) +
        tr("Latency: %1 ms").arg(response.latencyMs);
    QMessageBox::information(this, tr("Test Image Gen"), msg);
  } else {
    QString msg =
        tr("✗ Image Generation Failed\n\n") +
        tr("Error: %1").arg(QString::fromStdString(response.errorMessage));
    QMessageBox::warning(this, tr("Test Image Gen"), msg);
  }
}
