...
public:
    ...
    [[nodiscard]] DebugViewMode GetDebugViewMode() const noexcept { return m_debugViewMode; }

    void RequestPick(uint32_t x, uint32_t y) noexcept;
    void SetPickFlipY(bool enabled) noexcept { m_pickFlipY = enabled; }
    [[nodiscard]] PickResult GetLastPickResult() const noexcept { return m_lastPickResult; }
    void ClearPickResult() noexcept { m_lastPickResult.valid = false; }

    [[nodiscard]] FrameStats GetLastFrameStats() const noexcept { return m_lastFrameStats; }
...
private:
    ...
    void RecordOpaquePass(VkCommandBuffer cb, const std::vector<DrawInstance>& instances);
    void RecordPickingPass(VkCommandBuffer cb, const std::vector<DrawInstance>& instances);
