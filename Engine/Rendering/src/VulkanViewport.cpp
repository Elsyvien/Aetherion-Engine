...
void VulkanViewport::CreateDescriptorSetLayout() {
  ...
  void VulkanViewport::CreateSceneResources() {
    ...
    for (uint32_t i = 0; i < kMaxFramesInFlight; ++i) {
      CreateImage(
          gpu, device, m_swapchainExtent.width, m_swapchainExtent.height,
      ...
      CreateImage(gpu, device, m_swapchainExtent.width,
  ...
  void VulkanViewport::CreatePickingResources() {
    ...
    for (uint32_t i = 0; i < kMaxFramesInFlight; ++i) {
      ...
      m_pickingViews[i] =
          CreateImageView(device, m_pickingImages[i], m_pickingFormat,
                          VK_IMAGE_ASPECT_COLOR_BIT);
      ...
      m_pickingDepthViews[i] = CreateImageView(device, m_pickingDepthImages[i],
                                               m_depthFormat, aspect);
  ...
  void VulkanViewport::CreateDescriptorPoolAndSets() {
    ...
    for (uint32_t i = 0; i < kMaxFramesInFlight; ++i) {
      ...
      if (vkCreateDescriptorPool(device, &pool, nullptr,
                                 &m_descriptorPools[i]) != VK_SUCCESS) {
  ...
  void VulkanViewport::CreateFramebuffers() {
    ...
    for (uint32_t i = 0; i < kMaxFramesInFlight; ++i) {
      ...
      VkImageView pickAttachments[] = {m_pickingViews[i],
  ...
  void VulkanViewport::RecordCommandBuffer(
    ...
    auto recordPass = [&](uint32_t passIndex, auto &&fn) {
      if (m_timestampsSupported &&
          m_queryPools[m_frameIndex] != VK_NULL_HANDLE) {
    ...
    if (!instances.empty()) {
      ...
      for (const auto &instance : instances) {
        ...
        vkCmdPushConstants(
            ...
            sizeof(InstancePushConstants), &instance.constants);
  ...
  void VulkanViewport::RecordPickingPass(
    ...
    if (instances.empty()) {
      if (!m_pendingPick.pending &&
          m_debugViewMode != DebugViewMode::EntityId) {
        return;
      }
    }
    ...
    if (needsReadback || needsSampling) {
      ...
      toTransfer.dstAccessMask = needsReadback ? VK_ACCESS_TRANSFER_READ_BIT
    ...
    if (needsReadback &&
      ...
      vkCmdCopyImageToBuffer(cb, m_pickingImages[m_frameIndex],
                             VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                             m_pickingReadbackBuffers[m_frameIndex], 1,
                             &region);
  ...
  void VulkanViewport::RecordPostProcessPass(VkCommandBuffer cb,
    ...
    VkPipeline postPipeline = m_pickingFormatIsUint ? m_postProcessPipelineUint
    ...
    vkCmdBindDescriptorSets(cb, VK_PIPELINE_BIND_POINT_GRAPHICS,
                            m_postProcessPipelineLayout, 0, 2, sets, 0,
                            nullptr);
  ...
  void VulkanViewport::UpdateUniformBuffer(uint32_t frameIndex,
    ...
    if (useSceneCamera) {
      ...
      if (view.camera.projectionType == 1) {
        ...
        Mat4Ortho(proj, -halfWidth, halfWidth, -halfHeight, halfHeight,
                  nearPlane, farPlane);
      } else {
        const float fovRad =
    ...
    if (!primaryDirectional.enabled) {
      for (const auto &candidate : view.lights) {
        if (candidate.type == RenderLightType::Directional &&
            candidate.enabled && candidate.intensity > 0.0f) {
    ...
    auto pushLight = [&](const RenderLight &light) {
      ...
      float dir[3] = {light.direction[0], light.direction[1],
                      light.direction[2]};
      Vec3Normalize(dir);
  ...
  void VulkanViewport::UpdateSelectionBuffer(
    ...
    if (!hasModel) {
      ...
      auto modelFor = [&](auto &&self,
        ...
        const float radiansX = transform->GetRotationXDegrees() *
                               (3.14159265358979323846f / 180.0f);
        const float radiansY = transform->GetRotationYDegrees() *
                               (3.14159265358979323846f / 180.0f);
        const float radiansZ = transform->GetRotationZDegrees() *
                               (3.14159265358979323846f / 180.0f);
    ...
    auto addArrow = [&](const float dir[3], const float color[4]) {
      float end[3] = {origin[0] + dir[0] * axisLen,
      ...
      float baseCenter[3] = {end[0] - dir[0] * headLen,
                             end[1] - dir[1] * headLen,
                             end[2] - dir[2] * headLen};
  ...
  void VulkanViewport::UpdateLightGizmoBuffer(const RenderView &view) {
    ...
    auto addArrow = [&](const float origin[3], const float dirIn[3],
      ...
      float base[3] = {end[0] - dir[0] * headLength,
  ...
  void VulkanViewport::UpdateColliderBuffer(const RenderView &view) {
    ...
    auto addCircle = [&](const float center[3], const float right[3],
      ...
      for (int i = 0; i < segments; ++i) {
        float a0 =
            static_cast<float>(i) * kTwoPi / static_cast<float>(segments);
        float a1 =
    ...
    auto addHalfCircle = [&](const float center[3], const float right[3],
                             const float up[3], float radius,
                             const float color[4], bool topHalf,
                             int segments = 12) {
    ...
    for (const auto &collider : view.colliders) {
      ...
      if (collider.shapeType == 0) {
        ...
        addHalfCircle(topCenter.data(), rightXY, upXY, scaledRadius, color,
                      true);
  ...
  void VulkanViewport::SetCameraRotation(float yawDeg,
  ...
  void VulkanViewport::FocusOnBounds(float centerX, float centerY,
  ...
  void VulkanViewport::UpdateMetalLayerSize(int width, int height) {
    ...
    double scale = 1.0;
    id window =
        reinterpret_cast<id (*)(id, SEL)>(objc_msgSend)(view, selWindow);
    if (window) {
    ...
    reinterpret_cast<void (*)(id, SEL, CGRect)>(objc_msgSend)(
        layer, selSetBounds, bounds);
    reinterpret_cast<void (*)(id, SEL, CGRect)>(objc_msgSend)(
        layer, selSetFrame, bounds);
  }
#endif

  std::vector<VulkanViewport::DrawInstance> VulkanViewport::InstancesFromView(
      const RenderView &view, float timeSeconds) const {
    ...
    const std::unordered_map<
        Core::EntityId, const Scene::MeshRendererComponent *> *meshLookupPtr =
        &view.meshes;
    ...
    auto modelFor = [&](auto &&self,
      ...
      float localModel[16];
      Core::Math::Mat4Compose(localModel, transform->GetPositionX(),
                              transform->GetPositionY(),
                              transform->GetPositionZ(), radiansX, radiansY,
                              radiansZ, transform->GetScaleX(),
                              transform->GetScaleY(), transform->GetScaleZ());
    ...
    if (view.showEditorIcons) {
      ...
      if (view.camera.enabled) {
        ...
        const float yawRad =
            m_cameraYawDeg * (3.14159265358979323846f / 180.0f);
        const float pitchRad =
            m_cameraPitchDeg * (3.14159265358979323846f / 180.0f);
        const float distance = std::max(0.01f, m_cameraDistance * m_cameraZoom);
        const float eyeX =
      ...
      for (const auto &light : view.lights) {
        ...
        float pos[3] = {light.position[0], light.position[1],
                        light.position[2]};
  ...
  const VulkanViewport::GpuMesh *VulkanViewport::ResolveMesh(
      const std::string &assetId) {
    if (assetId.empty() || !m_context || !m_context->IsInitialized()) {
      return nullptr;
    }
    if (assetId == kIconMeshId) {
      return (m_iconMesh.vertexBuffer != VK_NULL_HANDLE) ? &m_iconMesh
                                                         : nullptr;
    }
    ...
    try {
      ...
      const VkDeviceSize indexSize =
          sizeof(std::uint32_t) *
          static_cast<VkDeviceSize>(indexSource->size());
  ...
  void VulkanViewport::CopyBufferToImage(VkBuffer buffer, VkImage image,
    ...
    if (vkAllocateCommandBuffers(m_context->GetDevice(), &alloc, &cmd) !=
        VK_SUCCESS) {
      throw std::runtime_error(
          "Failed to allocate texture copy command buffer");
    }
  ...
  std::vector<char> VulkanViewport::ReadFileBinary(const std::string &path)
      const {
  ...
  VkShaderModule VulkanViewport::CreateShaderModule(
    ...
    if (vkCreateShaderModule(m_context->GetDevice(), &create, nullptr,
                             &module) != VK_SUCCESS) {
